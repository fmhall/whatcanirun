CREATE MATERIALIZED VIEW "public"."view__model_device_summary" AS (select "models"."id" as "model_id", MIN("models_info"."family_id") as "family_id", "devices"."chip_id", MIN("devices"."cpu") as "device_cpu", MIN("devices"."cpu_cores") as "device_cpu_cores", MIN("devices"."gpu") as "device_gpu", MIN("devices"."gpu_cores") as "device_gpu_cores", MIN("devices"."ram_gb") as "device_ram_gb", AVG("trials"."decode_tps") as "avg_decode_tps", AVG("trials"."prefill_tps") as "avg_prefill_tps", (
          CASE
          WHEN AVG(
