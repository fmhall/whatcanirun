...
export async function processBundle(input: ProcessBundleInput): Promise<ProcessBundleResult> {
  const { bundleFile, ip, userId = null, did = null } = input;
  ...
  if (!manifestBytes) {
    return { ok: false, error: 'Missing `manifest.json` in bundle.', status: 400 };
  }
  ...
  if (manifestErrors.length > 0) {
    return { ok: false, error: 'Invalid manifest', details: manifestErrors, status: 400 };
  }

  // Extract and validate `results.json`.
  const resultsBytes = files['results.json'];
  if (!resultsBytes) {
    return { ok: false, error: 'Missing `results.json` in bundle.', status: 400 };
  }
  ...
  try {
    ...
    return { ok: false, error: 'Invalid `results.json`.', status: 400 };
  }

  const resultsErrors = validateResults(results);
  if (resultsErrors.length > 0) {
    return { ok: false, error: 'Invalid results', details: resultsErrors, status: 400 };
  }
  ...
  if (!device) {
    return { ok: false, error: 'Failed to resolve device.', status: 500 };
  }
  ...
  if (!model) {
    return { ok: false, error: 'Failed to resolve model.', status: 500 };
  }
  ...
  const [run] = await db
    ...
    .values({
      ...
      notes: truncate(manifest.notes, 5_000),
      userId,
      did,
      deviceId: device.id,
      modelId: model.id,
  ...
  if (!run) {
    return { ok: false, error: 'Failed to insert run.', status: 500 };
  }
